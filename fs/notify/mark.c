...
{
	struct fsnotify_group *current_group = NULL;
	int type;

	/*
	 * Get the group of the current iteration step from a mark that was
	 * selected for reporting.
	 */
	fsnotify_foreach_obj_type(type) {
		if (fsnotify_iter_should_report_type(iter_info, type)) {
			current_group = iter_info->marks[type]->group;
			break;
		}
	}
